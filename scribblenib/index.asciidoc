...
A party has a **travel budget** of 8 leagues each day.
Each hex is 1 league (3 miles) across as the crow flies,
but the party typically does not fly,
...
* **Difficult Terrain (3 leagues)**: Mountains, bogs, jungle; Disadvantage on WIL rolls made to navigate
...
* **Heavy rain, thick fog**: Disadvantage on WIL rolls made to navigate or search
* **Travel cautiously**: -2 to travel budget, Advantage on WIL rolls made to navigate or search
...
Each party member consumes 1 ration and 1 water each day.

A WIL roll is often needed to navigate through a hex.
No roll is necessary if the party has a map or guide, recognizes a landmark,
or is following a road, trail, river, or coastline to the next hex.
An unsuccessful roll results in a random encounter and/or getting lost.
A lost party can move to an adjacent hex
in the hopes of finding a recognizable landmark or someone to guide them,
or they can search the hex they're in to get their bearings again.

//TODO: merge paragraphs above and below

//TODO: replace "random encounter or lost" w/ random encounter table that
//      includes getting lost? replace WIL rolls too?

//d12 roll: 1-9 = environmental feature (consult d100 table) 10 = abnormal hazard (consult d8 table), 11 = hostile encounter (d100 table) and 12 is hostile encounter during an abnormal hazard (consult both).

To search, the party pays the movement cost of the hex they're in
and makes a WIL roll.
An unsuccessful roll results in a random encounter and/or getting lost.
They do one of the following if successful:

* Get their bearings again
* Reveal all landmarks and a hidden feature if one exists
* Forage -- each party member gains 1 ration or water

The terrain of each adjacent hex is visible under normal conditions.
Climbing a tall landmark reveals large landmarks in adjacent hexes
and increases the range of visible terrain by 1 hex in all directions.

When camping for the night in wilderness,
a WIL roll is often needed to keep watch.
The roll has Disadvantage if all party members are sleeping.
An unsuccessful roll results in a random encounter.
